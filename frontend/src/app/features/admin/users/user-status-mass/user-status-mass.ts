...
export class UserStatusMass {
  ...
  async onDeactivate(fila: any): Promise<void> {
    ...
    try {
      await firstValueFrom(this.userService.deactivate(fila.id));
  ...
  async onActivate(fila: any): Promise<void> {
    ...
    try {
      await firstValueFrom(this.userService.activate(fila.id));
      this.alertService.success(
        'Usuario activado',
  ...
  async operar(): Promise<void> {
    ...
    try {
      ...
      const requests = ids.map((id) =>
        firstValueFrom(
          willActivate ? this.userService.activate(id) : this.userService.deactivate(id),
        ),
      );
